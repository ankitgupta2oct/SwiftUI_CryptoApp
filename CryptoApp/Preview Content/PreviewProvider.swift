...
class DeveloperPreview {
    ...
    // MARK: Market Data
    lazy var stat1 = StatisticsModel(title: "Market Cap", value: "$15.7Bn", percantageValue: 0)
    lazy var stat2 = StatisticsModel(title: "Total Volume", value: "342Tr")
    lazy var stat3 = StatisticsModel(title: "Portfolio Value", value: "$125K", percantageValue: -32.65)
    ...
    func loadCoinData() {
        guard let url = Bundle.main.url(forResource: "CoinData", withExtension: "json")
            else {
    ...
    func loadCoinDetailData() {
        guard let url = Bundle.main.url(forResource: "CoinDetailData", withExtension: "json")
            else {
                print("Json file not found")
                return
            }
        ...
        let data = try? Data(contentsOf: url)
        let decodedData = try? JSONDecoder().decode(CoinDetailModel.self, from: data!)
